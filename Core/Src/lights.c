...
lightThreeshold naturalLightLevel = Low; // Natural light level initialization
...
void readNaturalLight ()
{
	...
	HAL_ADC_Start_IT(&hadc2); // Photoresistence value update
...
void setLight(_Bool check)
{
	if (check == 1)
	{
		// Switch to set every led with the respective color
		for(int i = 0; i<LEDUsed; i++)
		{
			switch(i){
			case(0):
